...
interface ToolContentProps {
  ...
  pageContent: { title: string; body: string } | null;
}

export function ToolContent({ slug, toolInfo, pageContent }: ToolContentProps) {
  const ToolComponent = TOOL_COMPONENTS[slug] ?? (slug === "tic-tac-toe" ? TicTacToe : null);
        ...
        >
          <div className="mb-6 border-b border-border/50 pb-4">
            ...
            <p className="text-muted-foreground text-[11px] md:text-xs mt-2 max-w-3xl leading-relaxed">
              {toolInfo.description} All computations are performed locally for
              maximum privacy.
            </p>
          </div>

          <div className={slug === "tic-tac-toe" ? "min-h-[320px]" : "min-h-[500px]"}>
            <ToolComponent />
          </div>

          {pageContent && (
            <section className="mt-8 pt-8 border-t border-border/50">
              <h3 className="text-lg font-bold mb-4">{pageContent.title}</h3>
              <div className="prose prose-sm dark:prose-invert max-w-none text-muted-foreground prose-p:leading-relaxed prose-headings:text-foreground prose-strong:text-foreground">
